use crate::event::{Event, Modifiers, MouseButton, pop_all_events, push_event};
use crate::ffi::{self, Class, YES, id};
use crate::objc;
...
impl EventLoop {
    pub fn new() -> Self {
        ...
        unsafe {
            ...
            objc::msg_send_id(
                ns_app,
                ffi::sel_registerName(b"finishLaunching\0".as_ptr() as *const _),
            );
    ...
    pub fn poll_events(&self) -> Vec<Event> {
        ...
        unsafe {
            ...
            let next_event_func: unsafe extern "C" fn(id, ffi::SEL, u64, id, id, ffi::BOOL) -> id =
                std::mem::transmute(ffi::objc_msgSend as *const std::ffi::c_void);
    ...
    unsafe {
        ...
        match event_type {
            ...
            | ffi::NSEventTypeRightMouseUp => {
                ...
                let window = objc::msg_send_id(
                    ns_event,
                    ffi::sel_registerName(b"window\0".as_ptr() as *const _),
                );
                if !window.is_null() {
                    let content_view = objc::msg_send_id(
                        window,
                        ffi::sel_registerName(b"contentView\0".as_ptr() as *const _),
                    );
                ...
                if event_type == ffi::NSEventTypeLeftMouseDown
                    || event_type == ffi::NSEventTypeRightMouseDown
                {
...
pub fn register_delegate_class() -> Class {
    ...
    REGISTER_DELEGATE.call_once(|| unsafe {
        ...
        cls = ffi::objc_allocateClassPair(
            superclass,
...
extern "C" fn window_did_resize(_this: id, _cmd: ffi::SEL, notification: id) {
    unsafe {
        let window: id = objc::msg_send_id(
            notification,
            ffi::sel_registerName(b"object\0".as_ptr() as *const _),
        );
        let content_view = objc::msg_send_id(
            window,
            ffi::sel_registerName(b"contentView\0".as_ptr() as *const _),
...
pub fn assert_main_thread() {
    unsafe {
        let thread_class = ffi::objc_getClass(b"NSThread\0".as_ptr() as *const _);
        let is_main_sel = ffi::sel_registerName(b"isMainThread\0".as_ptr() as *const _);
        let func: unsafe extern "C" fn(id, ffi::SEL) -> ffi::BOOL =
            std::mem::transmute(ffi::objc_msgSend as *const std::ffi::c_void);
